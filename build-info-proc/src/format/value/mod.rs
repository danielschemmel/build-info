...
pub(crate) trait Value: Debug {
	fn call_base(&self, func: &str, args: &[Box<dyn Value>]) -> manyhow::Result<Box<dyn Value>> {
		match func {
			OP_FIELD_ACCESS => {
			...
			OP_TUPLE_INDEX => Err(error_message!("Type {} cannot be tuple-indexed", self.get_type()).into()),
			OP_ARRAY_INDEX => Err(error_message!("Type {} cannot be indexed", self.get_type()).into()),
			_ => Err(
				error_message!(
...
fn as_arguments_0(args: &[Box<dyn Value>]) -> manyhow::Result<()> {
	if args.is_empty() {
		Ok(())
	} else {
		Err(error_message!("Wrong number of arguments (should be 0)").into())
	}
}

fn as_simple_arguments_1<T1: 'static>(args: &[Box<dyn Value>]) -> manyhow::Result<(&T1,)> {
	if args.len() != 1 {
		return Err(error_message!("Wrong number of arguments (should be 1)").into());
	}

	Ok((args[0]
		.as_any()
		.downcast_ref::<T1>()
		.ok_or_else(|| error_message!("Argument #1 should have type {}", type_name::<T1>()))?,))
}

fn as_named_arguments_1<T1: 'static>(args: &[(Option<String>, Box<dyn Value>)]) -> manyhow::Result<(&T1,)> {
	if args.len() != 1 {
		return Err(error_message!("Wrong number of arguments (should be 1)").into());
	}

	if args[0].0.is_some() {
		return Err(error_message!("Expected a single positional argument, found a named argument instead").into());
	}
